...
static client_info_t client = {"client", client_priv, sizeof(client_priv), client_pub, sizeof(client_pub), NULL, {}};
static client_info_t server = {"server", server_priv, sizeof(server_priv), server_pub, sizeof(server_pub), NULL, {}};
...
static void server_function(void)
{
	...
	if (bytes_received > 0)
	{
		...
		{
			/* Check whether data was indeed encrypted (it should not be the same as in data_to_send) */
			testsuite_fail_if(((size_t)bytes_sent == current_length) || (!memcmp(recv_buf, shared_mem, bytes_sent)), "secure_session server message wrap");
		}
		else
		{
			testsuite_fail_if(true, "secure_session_send failed");
